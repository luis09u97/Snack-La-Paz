...
fun SnackLaPazApp() {
    ...
    Surface(modifier = Modifier.fillMaxSize(), color = MaterialTheme.colorScheme.background) {
        if (showSplash) {
            ...
            // Placeholder temporário — na próxima etapa isso vira a navegação real
            // (bottom bar + Home + Carrinho + Pedidos + Perfil)
            Text(text = "Home em construção 🚧", modifier = Modifier.fillMaxSize())
        }
    }
}